...
archivo_excel = 'Resultados_Precedencias.xlsx'
...
print("\n" + "⭐"*30)
print(f"🚀 INICIANDO LOTE: {total_instancias} instancias detectadas")
print("⭐"*30 + "\n")
...
for indice, nombre_archivo in enumerate(archivos_txt, start=1):
    ...
    Modelo_precedencias.setParam('OutputFlag', 0)  # Gurobi en modo silencioso

    # --- Variables de decisión ---
    α = Modelo_precedencias.addVars(trabajos, trabajos, vtype=GRB.BINARY, name="alfa")
    ...
    for i in range(trabajos):
        for k in range(i + 1, trabajos):
            Modelo_precedencias.addConstr(α[i, k] + α[k, i] == 1, name=f"prec_{i}_{k}")
    ...
    if Modelo_precedencias.status in [GRB.OPTIMAL, GRB.TIME_LIMIT]:
        ...
        tiempo_anterior = Modelo_precedencias.Runtime
        gap_actual = Modelo_precedencias.MIPGap * 100
        
        nuevo_registro = pd.DataFrame({
            'Instancia': [nombre_archivo],
            'Mejor Solucion (Z)': [Modelo_precedencias.objVal],
        ...
        print(f"  ⚠️ La instancia no encontró solución. Estado: {Modelo_precedencias.status}\n")
        tiempo_anterior = Modelo_precedencias.Runtime if hasattr(Modelo_precedencias, 'Runtime') else 0
    
    # --- Liberar memoria ---
    Modelo_precedencias.dispose()
